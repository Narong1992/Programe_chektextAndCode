        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 self-end md:self-center">
             <div className="flex gap-2 text-xs font-medium">
                <div className="px-2 py-1 bg-green-50 text-green-700 border border-green-200 rounded">ตรงกัน: {stats.match}</div>
                <div className="px-2 py-1 bg-red-50 text-red-700 border border-red-200 rounded">ไม่ตรง: {stats.mismatch}</div>
                <div className="px-2 py-1 bg-yellow-50 text-yellow-700 border border-yellow-200 rounded">ขาด: {stats.missing}</div>
                <div className="px-2 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded">เกิน: {stats.extra}</div>
             </div>
             <button 
                onClick={handleAiAnalysis}
                disabled={isAnalyzing || (!sourceInput && !checkInput)}
                className={`flex items-center gap-2 px-4 py-2 rounded-md font-semibold text-white text-sm transition-all shadow-sm
